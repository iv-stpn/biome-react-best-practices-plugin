// Test harness for the react best-practices plugin.
//
// This plugin only reports diagnostics (no auto-fix), so each fixture is a pair:
//   tests/fixtures/<name>.tsx           — the source to lint
...
// `[react/<slug>]` prefix of each diagnostic message. An empty array asserts
...
for (const name of cases) {
  count++;
  const source = readFileSync(join(FIXTURES, `${name}.tsx`), "utf8");
  const expected = JSON.parse(readFileSync(join(FIXTURES, `${name}.expected.json`), "utf8"));

  const tmpFile = join(TMP, `${name}.tsx`);
